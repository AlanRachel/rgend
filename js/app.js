...
function updateSelectedAPIs() {
    ...
    const customApiIndicesFromDom = Array.from(customApiCheckboxes).map(input => 'custom_' + input.dataset.customIndex);

    // 如果自定义API列表未渲染（DOM中没有任何自定义checkbox），
    // 保留内存/localStorage中已有的自定义API选中状态，避免错误覆盖
    const hasRenderedCustomApis = document.querySelectorAll('#customApisList input[type="checkbox"]').length > 0;
    const customApiIndices = hasRenderedCustomApis
        ? customApiIndicesFromDom
        : JSON.parse(localStorage.getItem('selectedAPIs') || '[]').filter(id => id.startsWith('custom_'));
